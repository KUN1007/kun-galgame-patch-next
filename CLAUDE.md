...
- Catalog **reads and catalog edit** go through public API v2 (`pkg/catalogv2`): `KUN_NEXTMOE_API_BASE` is the origin (no `/v1` suffix); send `Authorization: Bearer nmk_…` (problem+json, string ids, cursor). User edits hit `/v2/me/proposals` and `/v2/moderation/snapshots`. Owner claim writes try `POST /v2/me/claims` and `PATCH {state: live|pending|withdrawn}` (D35a) and fall back to user-plane v1 when that catalog is still preview-only. Merged-proposal count tries `GET /v2/catalog/proposals` (D35b) and falls back to `GET /api/v1/catalog/edit/proposals`. `nm_test_` / `nm_live_` keys 401 on `/v2`. **Every v2 read must name what it wants**: a detail face answers a bare id+name row for each block the request does not list in `include=`, and `spoiler=` defaults to `none` — both render an empty page instead of erroring, which is how the first cutover silently dropped 会社 logos, 声优, rating histograms, spoilered tags and the whole character/staff modal. The token lists live beside the calls in `pkg/catalogv2`; an unknown token is a 400, so they track catalog's `apiv2/collect` specs. Always-v1 catalog calls: claim-events feed (D35d), new-work submit (`POST /api/v1/user/catalog/works/submit` — v2 mint requires refs and drops the wizard field map), and `GET /api/v1/user/catalog/claims/mine` (v2 list has no `product_work_id` / decline reason / `claim_state=`). Do not reintroduce the v1 envelope client or `X-API-Key` on `/v2`.
...
- **Catalog claims (C5 successor)**: catalog owns claim lifecycle events. Synchronization consumes `GET /api/v1/catalog/claim-events/feed`; never add a new consumer of the retired wiki message feed.
