...
- Catalog **reads and catalog edit** go through public API v2 (`pkg/catalogv2`): `KUN_NEXTMOE_API_BASE` is the origin (no `/v1` suffix); send `Authorization: Bearer nmk_…` (problem+json, string ids, cursor). User edits hit `/v2/me/proposals` and `/v2/moderation/snapshots`. `nm_test_` / `nm_live_` keys 401 on `/v2`. The remaining `/api/v1` catalog calls are the claim-events feed, merged-proposal count, and user claim/publish/withdraw (`pkg/catalogclient`) — v2 has no publish action and no claim-events feed. Do not reintroduce the v1 envelope client or `X-API-Key` on `/v2`.
