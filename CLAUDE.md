...
13. **One task = one Codex session = one branch = one worktree.** Never let two sessions write this checkout; prefer `codex-session new kun-galgame-patch <session>`.
...
- The `w161-p4` line moves publish/claim/withdraw and the cron inbox to catalog. Read `../kun-galgame-infra/refs/proj/161-n5-grand-window.md` and verify the branch, migration `029_claim_event_processed`, client binding, and deployment state before assuming the window ran.
