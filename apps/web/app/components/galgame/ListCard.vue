...
// first on one row and 汉化 first on the next.
...
// A one-column card is barely wider than its poster, and the full shelf wraps
// to three lines there while the poster beside it stops after one.
...
<template>
  <KunCard
    is-hoverable
    class-name="@container/card h-full w-full p-0"
    content-class="h-full gap-0 p-0"
  >
    <div class="flex h-full gap-3 p-3">
      <div
        class="relative w-24 shrink-0 overflow-hidden rounded-md sm:w-28"
          ...
          <NuxtLink
            ...
            :class="
              cn(
                ...
                index >= NARROW_TAG_LIMIT && 'hidden @md/card:block'
              )
            "
          >
            {{ tag.name }}
