...
// rather than guessing which company the caller meant. And, like its sibling,
// it ANSWERS when it cannot forward — rendering 200 over a target that does not
// exist is a soft 404, which keeps a dead URL indexed instead of retiring it.
//
...
if (verdict?.code === 0 && verdict.catalogId > 0) {
  await navigateTo(
    `/labels/${verdict.catalogId}${route.query.page ? `?page=${route.query.page}` : ''}`,
    { redirectCode: 301 }
  )
} else {
